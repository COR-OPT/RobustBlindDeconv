...
function main()
    ...
    @add_arg_table s begin
        ...
        "--prob_type"
            help =
                ...
                "randomized Hadamard matrices. 'pdft' results in problems " *
                "where the left measurement matrix is the partial DFT matrix " *
                "and the right matrix is Gaussian."
            range_tester = (x -> lowercase(x) in [
                "gaussian", "hadamard", "pdft"])
            default = "gaussian"
        "--method"
    ...
    (nfail -> begin
        if prob_type == "gaussian"
            BlindDeconv.genProblem(i * 2 * d, d, d, BlindDeconv.gaussian,
                                   BlindDeconv.gaussian, nfail, anoise)
        elseif prob_type == "hadamard"
            BlindDeconv.genProblem(i * 2 * d, d, d, BlindDeconv.randhadm,
                                   BlindDeconv.randhadm, nfail, anoise)
        elseif prob_type == "pdft"
            BlindDeconv.genProblem(i * 2 * d, d, d, BlindDeconv.pdft,
                                    BlindDeconv.complex_gaussian, nfail, anoise)
        else
            throw(ErrorException("prob_type must be one of {gaussian, hadamard, pdft}"))
        end end)
    solver =  # setup problem solver
        if method == "subgradient"
            (p -> BlindDeconv.subgradientMethod(p, λ, q, iters, ϵ=sdist))
        else
            (p -> BlindDeconv.proximal_method(p, 1, iters, ϵ=sdist))
        end
    if anoise
