...
function run_decay_eval(c::Int, pfail::Float64, reps::Int)
    ...
    for idx in 1:20
        @printf("Testing q: %.3f\n", q[idx])
        for rep in 1:reps
            prob = BlindDeconv.genProblem(
                m, d1, d2, BlindDeconv.gaussian, BlindDeconv.gaussian, pfail)
            _, _, ds = BlindDeconv.subgradientMethod(prob, 1.0, q[idx], 1000)
    ...
    title_string = "Avg. error, " * latexstring("c = $c")
...
function run_synth_test(method, c, d1, d2, pfail, q, iters)
    m = c * (d1 + d2)
    prob = BlindDeconv.genProblem(m, d1, d2, BlindDeconv.gaussian,
                                  BlindDeconv.gaussian, pfail)
    if method == "proximal"
        _, _, ds = BlindDeconv.proximal_method(prob, 1, iters)
    else
        # use polyak step by default without noise
        _, _, ds = BlindDeconv.subgradientMethod(prob, 1.0, q, iters,
                                                 use_polyak=(pfail == 0.0))
    end
    yscale("log")
...
"""
    run_dft_test(method, c, d, pfail, q, iters)

Run a test using a partial DFT and a complex gaussian matrix, for signal
dimension `d` and noise `pfail`.
"""
function run_dft_test(c, d, pfail, q, iters)
    m = trunc(Int, c * d * log(d))
    prob = BlindDeconv.genProblem(m, d, d, BlindDeconv.pdft,
                                  BlindDeconv.complex_gaussian, pfail)
    _, _, ds = BlindDeconv.subgradientMethod(prob, 1.0, q, iters,
                                             use_polyak=(pfail == 0.0))
    yscale("log")
    semilogy(collect(1:length(ds)), ds)
    xlabel(L"$ k $"); title("Normalized error, c = $c")
    show()
end
...
function run_matvec_eval(method, c, reps)
    ...
    if method == "proximal"
        solver = (p -> BlindDeconv.proximal_method(p, 1, 15, ϵ=1e-5))
    else
        solver = (p -> BlindDeconv.subgradientMethod(p, 1, 0.98, 1000,
                                                     ϵ=1e-5))
    end
    for k = 1:6
...
function run_mnist(method, c, idx_w, idx_x, iters, pfail, q)
    ...
    # setup problem
    prob = BlindDeconv.genProblem(c * (d1 + d2), w_sig, x_sig,
    ...
    if method == "proximal"
        ...
        wk, xk, error_hist = BlindDeconv.subgradientMethod(prob, 1.0, q, iters)
    end
...
function main()
    ...
    @add_arg_table s begin
        ...
        "dft_test"
            help = """
                Set up a synthetic instance using a partial DFT matrix and evaluate
                the performance of the subgradient method."""
            action = :command
        "matvec_eval"
    ...
    @add_arg_table s["dft_test"] begin
        "--c"
            help = "The coefficient c in m = c * d * log(d)"
            required = true
            arg_type = Int
        "--pfail"
            help = "The fraction of measurements corrupted with additive noise"
    ...
    elseif parsed["%COMMAND%"] == "dft_test"
        sb = parsed["dft_test"]
        run_dft_test(sb["c"], sb["d"], sb["pfail"], sb["q"], sb["iters"])
